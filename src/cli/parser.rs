...
fn phlp() {
    ...
    println!("    {} {}", pad(&format!("{C}-q{RST}, {C}--queries{RST} {G}<EQ>{RST}"), 20), format!("Equations to plot {G}(repeatable){RST}"));
    println!("    {} {}", pad(&format!("{C}--xmin{RST} {G}<VAL>{RST}"), 20), "Min x-axis value");
    println!("    {} {}", pad(&format!("{C}--xmax{RST} {G}<VAL>{RST}"), 20), "Max x-axis value");
    println!("    {} {}", pad(&format!("{C}--ymin{RST} {G}<VAL>{RST}"), 20), "Min y-axis value");
    println!("    {} {}", pad(&format!("{C}--ymax{RST} {G}<VAL>{RST}"), 20), "Max y-axis value");
    println!("    {} {}", pad(&format!("{C}-h{RST}, {C}--help{RST}"), 20), "Show this help");
    println!("    {} {}", pad(&format!("{C}-v{RST}, {C}--version{RST}"), 20), "Show version");
    println!();
    println!("  {B}FORMAT{RST}");
    println!("    {G}y={RST}{B}<expression>{RST}");
    println!();
    println!("    {} {}", pad(&format!("{C}y=x^2{RST}"), 12), format!("{G}parabola{RST}"));
    println!("    {} {}", pad(&format!("{C}y=sin(x){RST}"), 12), format!("{G}sine wave{RST}"));
    println!("    {} {}", pad(&format!("{C}y=2x+1{RST}"), 12), format!("{G}linear{RST}"));
    println!("    {} {}", pad(&format!("{C}y=1/x{RST}"), 12), format!("{G}hyperbola{RST}"));
    println!("    {} {}", pad(&format!("{C}y=e^x{RST}"), 12), format!("{G}exponential{RST}"));
    println!();
